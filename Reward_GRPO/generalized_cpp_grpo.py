...
def _authenticated_pass_status(receipt: Mapping[str, Any]) -> str:
    ...
    if any(not isinstance(k, Mapping) or k.get("status") == "invalid" for k in kernels):
        return "invalid"
...
def _mutation_control_samples(registry: TaskRegistry, task_ids: list[str] | None = None) -> list[dict[str, Any]]:
    ...
    available = _curriculum_task_ids(registry)
    task_ids = available if task_ids is None else task_ids
    if not set(task_ids) <= set(available):
        raise BindingError("invalid_mutation_controls", "unknown selected training task")
    if not isinstance(entries, dict) or set(entries) != set(available):
        raise BindingError(
