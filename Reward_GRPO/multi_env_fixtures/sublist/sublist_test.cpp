...
TEST_CASE("named sublist outcomes are distinct", "[semantic-enum-identity]") {
    using Value = sublist::List_comparison;
    CHECK(Value::equal != Value::sublist);
    CHECK(Value::equal != Value::superlist);
    CHECK(Value::equal != Value::unequal);
    CHECK(Value::sublist != Value::superlist);
    CHECK(Value::sublist != Value::unequal);
    CHECK(Value::superlist != Value::unequal);
}
