...
TEST_CASE("Cannot deposit zero or the smallest negative amount", "[non-positive-deposit]") {
    Bankaccount::Bankaccount account;
    account.open();
    account.deposit(100);
    for (int amount : {0, std::numeric_limits<int>::min()}) {
        REQUIRE_THROWS_AS(account.deposit(amount), std::runtime_error);
        REQUIRE(account.balance() == 100);
    }
}

TEST_CASE("Cannot withdraw zero or the smallest negative amount", "[non-positive-withdrawal]") {
    Bankaccount::Bankaccount account;
    account.open();
    account.deposit(100);
    for (int amount : {0, std::numeric_limits<int>::min()}) {
        REQUIRE_THROWS_AS(account.withdraw(amount), std::runtime_error);
        REQUIRE(account.balance() == 100);
    }
}
