...
TEST_CASE("full signed int construction wraps without intermediate overflow")
{
    using Clock = date_independent::clock;
    for (int hour : {std::numeric_limits<int>::min(), std::numeric_limits<int>::max()}) {
        for (int minute : {std::numeric_limits<int>::min(), std::numeric_limits<int>::max()}) {
            // Reduce each input before addition; the expected clock uses bounded inputs.
            REQUIRE(string(Clock::at(hour, minute)) ==
                    string(Clock::at(hour % 24, minute % 1440)));
        }
    }
}

TEST_CASE("full signed int updates wrap without intermediate overflow")
{
    using Clock = date_independent::clock;
    for (int delta : {std::numeric_limits<int>::min(), std::numeric_limits<int>::max()}) {
        REQUIRE(string(Clock::at(0, 1).plus(delta)) ==
                string(Clock::at(0, 1 + delta % 1440)));
        REQUIRE(string(Clock::at(0, 0).minus(delta)) ==
                string(Clock::at(0, -(delta % 1440))));
        auto clock = Clock::at(23, 59);
        clock.plus(delta).minus(delta);
        REQUIRE(string(clock) == "23:59");
    }
}
