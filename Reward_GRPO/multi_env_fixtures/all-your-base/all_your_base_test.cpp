...
TEST_CASE("conversion preserves values wider than a machine integer") {
    vector<unsigned int> binary(129, 0);
    binary.front() = 1;
    vector<unsigned int> hexadecimal(33, 0);
    hexadecimal.front() = 1;
    REQUIRE(all_your_base::convert(2, binary, 16) == hexadecimal);
    REQUIRE(all_your_base::convert(16, hexadecimal, 2) == binary);
}

TEST_CASE("maximum unsigned bases preserve each digit") {
    const auto base = std::numeric_limits<unsigned int>::max();
    const vector<unsigned int> digits{base - 1, base - 1};
    REQUIRE(all_your_base::convert(base, digits, base) == digits);
}
