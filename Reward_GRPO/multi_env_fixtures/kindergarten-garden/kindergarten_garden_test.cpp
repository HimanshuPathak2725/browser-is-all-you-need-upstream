...
// These named results represent different contract outcomes. Do not assume
// any particular underlying integer values, but require distinguishable values.
TEST_CASE("named kindergarten-garden outcomes are distinct", "[semantic-enum-identity]") {
    using Value = kindergarten_garden::Plants;
    CHECK(Value::clover != Value::grass);
    CHECK(Value::clover != Value::radishes);
    CHECK(Value::clover != Value::violets);
    CHECK(Value::grass != Value::radishes);
    CHECK(Value::grass != Value::violets);
    CHECK(Value::radishes != Value::violets);
}

// Translate symbolic results to an independent representation. The expected
// answer is literal diagram meaning, not a candidate-defined enum array.
TEST_CASE("all four plant meanings remain distinguishable", "[semantic-plant-identity]") {
    using Plants = kindergarten_garden::Plants;
    const auto symbol = [](Plants plant) {
        if (plant == Plants::clover) return 'C';
        if (plant == Plants::grass) return 'G';
        if (plant == Plants::radishes) return 'R';
        if (plant == Plants::violets) return 'V';
        return '?';
    };
    const auto actual = kindergarten_garden::plants("CG\nRV", "Alice");
    const std::array<char, 4> meaning{symbol(actual[0]), symbol(actual[1]),
                                      symbol(actual[2]), symbol(actual[3])};
    const std::array<char, 4> expected{'C', 'G', 'R', 'V'};
    REQUIRE(meaning == expected);
}
