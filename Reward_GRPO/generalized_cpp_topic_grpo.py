...
CURRICULUM = "cpp-grpo-final-v1"
MODULE = "Reward_GRPO.generalized_cpp_topic_grpo"
IMAGE = "glm47-generalized-cpp-topics:final-v1"
REWARD_FILES = (
    ...
    "Reward_GRPO/generalized_cpp_cohort.py", "Reward_GRPO/cohorts/cpp-grpo-final-v1.json",
)
REWARD_DIRS = (
...
def worker_score(sample):
    ...
    cohort = load_cohort(registry=registry)
    if task_id not in cohort["train"] + cohort["validation"]:
        return compose(base._infrastructure_failure(sample, task_id,
            "task_not_admitted", "task is outside the frozen training/evaluation cohort"))
...
def build_data(args):
    from Reward_GRPO.generalized_cpp_cohort import build_data as export_cohort
    if args.curriculum != CURRICULUM:
        raise ValueError("training requires the explicitly versioned final cohort")
    return export_cohort(args)
...
def preflight(output=None, quick=False):
    ...
    if not quick:
        for sample in base._mutation_control_samples(registry, task_ids=cohort["train"]):
        ...
        for task in sorted(selected & set(TOPICS)):
            binding = registry.resolve(task)
        ...
        if "kindergarten-garden" in selected:
            ...
            sources = reference_sources(registry.resolve(task))
            for control in enum_controls(task, sources):
                add(task, response_from_sources(control.sources), "official_" + control.kind,
                    control.expected, control.name)
    ...
    result = {"status": "passed" if all(r["matched"] for r in records) else "failed",
              ...
              "train": cohort["train"], "validation": cohort["validation"],
              "topic_tasks": sorted(selected & set(TOPICS))}
    if output:
