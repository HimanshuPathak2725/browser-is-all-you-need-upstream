...
{
    const int raw = static_cast<int>((static_cast<long long>(hour_ % hours_per_day) * minutes_per_hour + minute_) % 1440);
...
def alternative(task: str, sources: dict[str, str]) -> dict[str, str]:
    ...
    if task == "bank-account":
        # Locking strategy is independent of the explicit non-positive rejection rule.
        text = sources["bank_account.cpp"]
        if text.count("std::lock_guard guard(mutex_);") != 3:
            raise ValueError("bank-account locking control anchors changed")
        return {**sources, "bank_account.cpp": text.replace(
    ...
    if task == "complex-numbers":
        return edit(sources, "complex_numbers.cpp",
                    "Complex other{scalar, 0};\n    return other / complex;",
                    "return (Complex(scalar, 0) / complex.conj()).conj();")
...
def enum_controls(task: str, sources: dict[str, str]) -> list[Control]:
    """Semantic enum controls also used by official-only Kindergarten Garden.

    Correct candidates may choose any distinct numeric representation. A
    constant result with aliased names must fail even when all official
    expected values were written using those same candidate-defined names.
    """
    if task == "sublist":
...
'''
    ...
    elif task == "kindergarten-garden":
        enum = "enum class Plants : char { grass = 'G', clover = 'C', radishes = 'R', violets = 'V' };"
        header = "kindergarten_garden.h"
        distinct = "enum class Plants : char { grass = 19, clover = 7, radishes = 42, violets = 101 };"
        collapsed = "enum class Plants : char { grass = 0, clover = 0, radishes = 0, violets = 0 };"
        aliased = "enum class Plants : char { grass = 0, clover = 1, radishes = 1, violets = 2 };"
        constant_source = '''#include "kindergarten_garden.h"
...
'''
        positive = edit(sources, header, enum, distinct)
        positive = edit(positive, "kindergarten_garden.cpp",
        ...
        raise ValueError("task does not have enum controls")
    constant = edit(sources, header, enum, collapsed)
    constant[task.replace("-", "_") + ".cpp"] = constant_source
    ...
    return [Control("nonstandard_enum_values", positive, "pass", "positive"),
            Control("collapsed_enum_constant", constant, "fail", "semantic",
                    "equal_relation" if task == "sublist" else None),
...
def controls(task: str, sources: dict[str, str]) -> list[Control]:
    ...
    if task == "perfect-numbers":
        ...
        mutation("narrow_addition", "clock.cpp",
                 "minute_ = static_cast<int>((static_cast<std::int64_t>(minute_) + minutes) % 1440);",
                 "minute_ += minutes;", "signed_updates")
        mutation("narrow_subtraction", "clock.cpp",
                 "minute_ = static_cast<int>((static_cast<std::int64_t>(minute_) - minutes) % 1440);",
                 "minute_ -= minutes;", "signed_updates")
        mutation("narrow_creation", "clock.cpp",
                 "static_cast<std::int64_t>(hour_) * minutes_per_hour + minute_",
                 "hour_ * minutes_per_hour + minute_", "canonical_creation")
    elif task == "bank-account":
        mutation("accepts_zero_amount", "bank_account.cpp", "if (amount <= 0)",
                 "if (amount < 0)", "rejected_state")
        mutation("wrong_nonpositive_exception", "bank_account.cpp",
                 'throw std::runtime_error{"amount must be greater than 0"};',
                 'throw std::invalid_argument{"amount must be greater than 0"};', "rejected_state")
        ...
        mutation("overflowing_magnitude", "complex_numbers.cpp", "std::hypot(re, im)",
                 "std::sqrt(re * re + im * im)", "identities_and_exponential")
        text = sources["complex_numbers.cpp"]
        start = text.index("    // Scale both operands")
        end = text.index("\n}", start)
        narrow = ("    return Complex{(re * other.re + im * other.im) / "
                  "(other.re * other.re + other.im * other.im), "
                  "(im * other.re - re * other.im) / "
                  "(other.re * other.re + other.im * other.im)};")
        result.append(Control("overflowing_division", {**sources,
                              "complex_numbers.cpp": text[:start] + narrow + text[end:]},
                              "fail", "semantic", "direct_divide"))
        mutation("overflowing_scalar_division", "complex_numbers.cpp",
                 "Complex other{scalar, 0};\n    return other / complex;",
                 ...
                 "-scalar*complex.imag()/denominator);", "scalar_divide")
        ...
        result.extend(enum_controls(task, sources))
