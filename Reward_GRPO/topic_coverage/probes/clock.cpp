    ...
    if (group == "canonical_creation") {
        ...
        for (int hour : {std::numeric_limits<int>::min(), std::numeric_limits<int>::max()})
            for (int minute : {std::numeric_limits<int>::min(), std::numeric_limits<int>::max()}) {
                coverage::context = "wide hour=" + coverage::show(hour) + " minute=" + coverage::show(minute);
                inspect(Clock::at(hour, minute), 60LL * hour + minute);
            }
        ...
        for (int delta : {std::numeric_limits<int>::min(), std::numeric_limits<int>::max()}) {
            coverage::context = "wide delta=" + coverage::show(delta);
            auto clock = Clock::at(0, 1);
            clock.plus(delta);
            inspect(clock, 1LL + delta);
            clock.minus(delta);
            inspect(clock, 1);
            clock = Clock::at(0, 0);
            clock.minus(delta);
            inspect(clock, -static_cast<long long>(delta));
        }
