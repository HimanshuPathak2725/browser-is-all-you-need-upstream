...
void run_topic(const std::string& group) {
    if (group == "initial_state") {
        ...
        for (int repeat = 0; repeat < 16; ++repeat) {
            ...
            operation(candidate, model, 0, 1);
            operation(candidate, model, 2, 0);
            operation(candidate, model, 3, 0);
            operation(candidate, model, 2, std::numeric_limits<int>::min());
            operation(candidate, model, 3, std::numeric_limits<int>::min());
