...
void run_topic(const std::string& group) {
    ...
    // Scaling the same nonzero operands leaves their quotient unchanged. These
    // cases have finite operands and representable results, without requiring
    // accuracy tighter than the existing normalized 0.005 tolerance.
    for (double scale : {1e200, 1e-200}) {
        const Complex left(scale, 2 * scale), right(3 * scale, 4 * scale);
        coverage::context = "finite scale=" + coverage::show(scale);
        if (group == "direct_divide")
            inspect(left / right, .44L, .08L, "scaled division");
        if (group == "scalar_divide") {
            inspect(right / scale, 3, 4, "scaled complex divided by scalar");
            inspect(scale / right, .12L, -.16L, "scaled scalar divided by complex");
        }
        if (group == "identities_and_exponential")
            coverage::close(right.abs() / scale, 5, "scaled magnitude");
    }
}
