...
checked after noncanonical construction and updates as well as every in-day time. Construction
and updates include `INT_MIN` and `INT_MAX`; the reference normalizes using wide arithmetic.
Negative controls independently reject narrow construction, addition and subtraction. The public header is included before
...
only; a clean sample does not prove race freedom. Zero and negative amounts must
throw `std::runtime_error` without changing the balance, as required by the explicit
interface contract. Controls reject zero acceptance and an incorrect exception type.
Concurrent open/close and balance arithmetic overflow are not graded.
...
The expected relation and group routing use trusted string labels, independent of
candidate enum values. Every group first requires the four named relation values
to be distinct. Distinct nonstandard numeric values remain valid; collapsed and
partially aliased enums are negative controls. These checks preserve the existing
bounded domain and reward-family denominators.

The official Sublist and Kindergarten Garden suites also enforce distinct named
outcomes. Kindergarten Garden checks a literal `C,G,R,V` meaning sequence as well
as the existing student diagrams. Its official-only controls live in
`controls.enum_controls`; no new topic or reward family is introduced. Both tasks
have nonstandard distinct-enum positive controls and constant/aliased-enum
negative controls, exercised through G03 and its reward adapter by
`tests/test_cpp_enum_semantics.py`. No particular enum integers are prescribed.
...
verdicts. Finite scales `1e200` and `1e-200` additionally check magnitude,
direct division and both scalar orderings using the same normalized 0.005
tolerance. Overflow/underflow controls must fail; the alternative positive control
uses a valid conjugation identity. Zero denominators and nonfinite inputs are excluded.
...
exact random sequence. Because this does not enforce the contract's random
generation requirement, Dnd-character is held out of full-correctness training
admission; a constant ability result must not be treated as a validated task
solution. Grade-school is likewise held while duplicate rejection lacks a defined
API mechanism. See [task repair notes](../task_repairs/numerical_contract_notes.md).
