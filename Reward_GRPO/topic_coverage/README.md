...
The v3 validation catalog contains 88 topic cases: 11 positive controls,
13 correct alternatives, 46 semantic faults, 17 build faults and one diagnostic.
Every semantic mutant must compile and fail its named requirement; a build error
cannot stand in for detection of a semantic defect. Correct alternatives include
constructor initialization and renamed private fields. See the v3 validation
report for executed results and archived-response replay evidence.
Historical model outputs are a separate hash-bound replay campaign:

~~~sh
PYTHONPATH=src python3 -B scripts/replay_cpp_topic_coverage.py \
  --evidence /path/to/pass1-failure-analysis/evidence.json \
  --output /tmp/topic-history-replay-NEW --allow-local-execution
~~~

Review the extracted code first. The replay validates history hashes and exact
code-block locations; missing edited files may only come from an archived
starter that matches the current trusted starter hash. It never repairs answers.

The pytest suite also checks an allergy high-bit mutant against the existing
generalized verifier and this extra layer, measuring actual additional coverage.
The validation --compare-generalized option saves that comparison with the campaign.

~~~sh
PYTHONPATH=src python3 -B -m pytest -q tests/test_topic_coverage.py
~~~
...
still-incorrect solution stays nonpositive. See `../GENERALIZED_CPP_TOPICS_GRPO.md`.
